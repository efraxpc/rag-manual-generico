...
un token de Azure AI Search que representa al mismo usuario. Search aplica los
roles RBAC del usuario o de sus grupos.

```text
Usuario -> Streamlit -> token access_as_user -> FastAPI
                                             -> intercambio OBO
                                             -> token de Search del usuario
                                             -> Azure AI Search
```
...
En **API permissions**, añade el permiso delegado de Azure AI Search
`user_impersonation` y concede consentimiento de administrador para el tenant.
OBO requiere ese consentimiento en el registro de la API.
...
Abre `http://localhost:8501`, inicia sesión y carga un archivo. `az login` no se
usa para las peticiones de la aplicación. Sigue siendo necesario para ejecutar
Terraform o el comando administrativo `python -m app.commands.prepare_text_index`.
...
entre sesiones. Streamlit fija la URL de FastAPI mediante `APP_API_BASE_URL` y
no permite al usuario cambiar el destino al que envía el bearer token.
